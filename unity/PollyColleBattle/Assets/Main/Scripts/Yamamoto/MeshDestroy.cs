...
public class MeshDestroy : MonoBehaviour
{
    ...
    [Header("ÉJÉbÉgêî"),Range(1,15)]
    public int CutCascades = 1;
    [Header("îjâÛóÕ")]
    public float ExplodeForce = 300;
    [Header("îjâÛå„Ç…è¡Ç¶ÇÈéûä‘")]
    public float partDestroyTime = 2.0f;
    
    
    //ÉÅÉbÉVÉÖÇï™äÑê∂ê¨ÇµÇƒîjâÛÇ∑ÇÈ
    public void DestroyMesh()
    {
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            parts[i].GameObject.layer = LayerMask.NameToLayer("GameEffect");
            StartCoroutine("partDelayDestroy", parts[i].GameObject);
        }

        GetComponent<MeshRenderer>().enabled = false;
        gameObject.layer = LayerMask.NameToLayer("GameEffect");
        StartCoroutine("partDelayDestroy", gameObject);
    }

    IEnumerator partDelayDestroy(GameObject obj)
    {
        yield return new WaitForSeconds(partDestroyTime);
        Destroy(obj);
    }
